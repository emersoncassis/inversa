...
export class Writer {
  ...
  async installEntryFile(engine, { force = false, conflictStrategy = 'prompt' } = {}) {
    ...
    const strategy = conflictStrategy === 'prompt'
      ? await askMergeStrategy(engine.entryFile)
      : conflictStrategy;
  ...
  createReversaDir(answers, version) {
    ...
    state.player ??= {};
    state.player.name = answers.user_name || 'Player';
    state.missions ??= {};
    if (state.missions['basic-reversa-journey']) {
      state.missions['basic-reversa-journey'].started_at = new Date().toISOString();
    }
