...
const CLI_ALIASES = new Set(['inversa', 'reversa']);
...
function parseReversaCommand(rawCommand) {
  ...
  if (!command) {
    return { ok: false, error: 'Digite um comando inversa ou reversa.' };
  }
  ...
  if (tokens[0] === 'npx' && CLI_ALIASES.has(tokens[1])) {
    tokens = tokens.slice(1);
  }

  if (!CLI_ALIASES.has(tokens[0])) {
    return { ok: false, error: 'Apenas comandos inversa ou reversa sao permitidos.' };
  }
