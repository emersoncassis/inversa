...
function syncActiveScreen(data) {
  if (!data.installed) return;

  const currentPhase = data.workflow.currentPhase || data.workflow.phases[0]?.id;
  if (!currentPhase) return;

  renderPhaseDetail(currentPhase);
  activateScreen('phase', currentPhase);
}
