...
const els = {
  ...
  startPhaseButton: document.querySelector('#startPhaseButton'),
  completePhaseButton: document.querySelector('#completePhaseButton'),
  phaseActionHint: document.querySelector('#phaseActionHint'),
...
els.startPhaseButton.addEventListener('click', () => updateWorkflowPhase('start'));
els.completePhaseButton.addEventListener('click', () => updateWorkflowPhase('complete'));
...
function fillInstallDefaults() {
  ...
  els.engineChoices.replaceChildren(...options.engines.map(engine => {
    ...
    label.innerHTML = `
      ...
      <span class="engine-logo" aria-hidden="true">
        <img src="${escapeAttribute(engineLogoPath(engine.id))}" alt="">
      </span>
...
function syncPhaseControls(phase) {
  const installed = Boolean(state.dashboard?.installed);
  const completed = phase.status === 'completed';
  const active = phase.status === 'active';
  const nextPhase = state.dashboard?.workflow.phases.find(item => item.status === 'active');

  els.startPhaseButton.disabled = !installed || completed || active;
  els.completePhaseButton.disabled = !installed || completed;

  if (!installed) {
    els.phaseActionHint.textContent = 'Instale o Inversa para liberar o controle das fases.';
  } else if (completed) {
    els.phaseActionHint.textContent = 'Passo concluido. Use o menu para revisar ou continuar a proxima fase.';
  } else if (active) {
    els.phaseActionHint.textContent = 'Passo em andamento. Conclua quando a saida esperada estiver validada.';
  } else if (nextPhase) {
    els.phaseActionHint.textContent = `Fase atual: ${nextPhase.title}. Voce ainda pode iniciar este passo manualmente.`;
  } else {
    els.phaseActionHint.textContent = 'Fluxo pronto para revisao final.';
  }
}
...
function engineLogoPath(engineId) {
  const logos = {
    ...
    aider: 'aider.png',
    'amazon-q': 'amazon-q.svg',
  };

  return `/assets/logos/${logos[engineId] ?? 'opencode.svg'}`;
}
...
async function updateWorkflowPhase(action) {
  if (!selectedPhaseId) return;

  const buttons = [els.startPhaseButton, els.completePhaseButton];
  buttons.forEach(button => {
    button.disabled = true;
  });

  try {
    const result = await postJson(`/api/workflow/${action}`, { phaseId: selectedPhaseId });
    state.dashboard = result.dashboard;
    render();
    const activePhaseId = action === 'complete'
      ? state.dashboard.workflow.currentPhase || selectedPhaseId
      : selectedPhaseId;
    openPhaseStep(activePhaseId);
  } catch (error) {
    els.phaseActionHint.textContent = error.message;
  } finally {
    const phase = state.dashboard?.workflow.phases.find(item => item.id === selectedPhaseId);
    if (phase) syncPhaseControls(phase);
  }
}
