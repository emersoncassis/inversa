...
import { completeWorkflowPhase, startWorkflowPhase } from './workflow.js';
...
async function routeRequest({ req, res, projectRoot, version }) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/workflow/start') {
    const payload = await readJsonBody(req);
    const state = startWorkflowPhase(projectRoot, payload.phaseId);
    return sendJson(res, 200, { state, dashboard: getDashboardState(projectRoot) });
  }

  if (req.method === 'POST' && url.pathname === '/api/workflow/complete') {
    const payload = await readJsonBody(req);
    const state = completeWorkflowPhase(projectRoot, payload.phaseId);
    return sendJson(res, 200, { state, dashboard: getDashboardState(projectRoot) });
  }
...
function sendStatic(res, fileName) {
  const cleanName = normalize(fileName).replace(/^(\.\.(\/|\\|$))+/, '');
  const filePath = resolve(STATIC_DIR, cleanName);
