...
import { buildLearningDashboard, ensureLearningState } from './gamification.js';
...
export function getDashboardState(projectRoot) {
  ...
  const state = installation.installed ? ensureLearningState(installation.state) : null;
  ...
  const learning = buildLearningDashboard(state, phases);
  ...
  return {
    ...
    gamification,
    learning,
    nextAction: resolveNextAction({ installation, state, outputExists, artifactStats, phases, learning }),
    actions: webActions,
  };
}
...
function resolveNextAction({ installation, state, outputExists, artifactStats, phases, learning }) {
  if (!installation.installed) {
    return {
      title: 'Instalar Inversa neste projeto',
      description: 'Comece a missao basica: acople o Reversa, configure engines e prepare sua jornada de aprendizagem.',
      action: 'install',
    };
  }

  if (learning?.mission?.status === 'completed') {
    return {
      title: 'Missao basica concluida',
      description: 'Voce ja aprendeu o fluxo inicial. Agora pode evoluir para projetos maiores usando o Reversa com mais autonomia.',
      action: 'advanced-reversa',
    };
  }
  ...
  if (activePhase) {
    return {
      title: `Continuar missao: ${activePhase.title}`,
      description: `Execute a fase ${state?.phase}, valide a saida e conclua o passo para ganhar XP.`,
  ...
  if (!outputExists || artifactStats.markdownFiles === 0) {
    return {
      ...
      description: 'Use o comando reversa no agente de IA configurado para gerar os primeiros artefatos da missao.',
      action: 'run-agent',
    };
  }
  ...
  return {
    ...
    description: 'Explore artefatos, lacunas e diagramas gerados no SDD para consolidar o aprendizado.',
    action: 'review',
  };
}

function calculateGamification({ installed, installedEngines, installedAgents, artifactStats, phases, state }) {
  ...
  const level = player?.level ?? Math.max(1, Math.floor(xp / 250) + 1);
  ...
  return {
    ...
    title: player?.rank ?? resolveRank(level),
    badges: [
      {
        id: 'first-setup',
        title: 'Base montada',
      ...
      {
        id: 'first-basic-project',
        title: 'Primeira missao',
        unlocked: player?.badges?.includes('first-basic-project') ?? false,
      },
    ],
  };
}
